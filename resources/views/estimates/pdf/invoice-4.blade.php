    ...
    <div class="right-invoice">
        ...
        @if(!is_null($estimate->project) && !is_null($estimate->project->client))
            <section id="client-info">
                ...
                <div class="client-name">
                    <strong>{{ ucwords($estimate->project->client->name) }}</strong>
                </div>
                ...
                <div class="mb-3">
                    <span>
                        ...
                        {!! nl2br($estimate->project->client->clientDetails->address) !!}
                    </span>
                </div>
                @if ($estimate->show_shipping_address === 'yes')
                    <div>
                        <span>
                            ...
                            {!! nl2br($estimate->project->client->clientDetails->shipping_address) !!}
                        </span>
                    </div>
                @endif
                @endif
        ...
        @elseif(!is_null($estimate->client_id))
            <section id="client-info">
                <span>@lang('modules.invoices.billedTo'):</span>
                <div class="client-name">
                    <strong>{{ ucwords($estimate->client->name) }}</strong>
                </div>
        ...
        @if(is_null($estimate->project) && !is_null($estimate->estimate) && !is_null($estimate->estimate->client))
            <section id="client-info">
                <span>@lang('modules.invoices.billedTo'):</span>
                <div class="client-name">
                    <strong>{{ ucwords($estimate->estimate->client->name) }}</strong>
                </div>
        ...
        <section id="invoice-info">
            @if($estimate->status == 'unpaid')
                <div>
                    <span>@lang('app.dueDate'):</span> <span>{{ $estimate->due_date ? $estimate->due_date->format($global->date_format) : '--' }}</span>
                </div>
            @endif
            <div>
