...
<div class="modal-body">
    <x-table class="table-bordered" headType="thead-light">
        ...
        @forelse($departments as $key=>$item)
            <tr id="row-{{ $item->id }}">
                <td>{{ $key + 1 }}</td>
                <td data-row-id="{{ $item->id }}" contenteditable="true">{{ ucwords($item->team_name) }}</td>
                <td class="text-right">
    ...
    <x-form id="createProjectCategory">
        <div class="row border-top-grey ">
            <div class="col-sm-12">
                <x-forms.text fieldId="department_name" :fieldLabel="__('app.name')" fieldName="department_name"
