            ...
            <x-cards.data padding="false" :title="__('app.newNotifications')">
                @forelse ($user->unreadNotifications as $key => $notification)
                    ...
                    @foreach ($worksuitePlugins as $item)
                        @if(View::exists(strtolower($item).'::notifications.'.$userType.'.'.\Illuminate\Support\Str::snake(class_basename($notification->type))))
                            @include(strtolower($item).'::notifications.'.$userType.'.'.\Illuminate\Support\Str::snake(class_basename($notification->type)))
                        @endif
                    @endforeach
