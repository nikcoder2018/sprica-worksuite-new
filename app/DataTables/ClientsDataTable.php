...
class ClientsDataTable extends BaseDataTable
{
    ...
    public function query(User $model)
    {
        ...
        $users = $model->withoutGlobalScope('active')->with('session', 'clientDetails')
            ...
            ->select('users.id', 'users.name', 'client_details.company_name', 'users.email', 'users.image', 'users.created_at', 'users.status', 'client_details.added_by', 'users.admin_approval')
