...
class InvoiceController extends AccountBaseController
{
    ...
    public function destroy($id)
    {
        ...
        abort_403(!($this->deletePermission == 'all' || ($this->deletePermission == 'added' && $invoice->added_by == user()->id)));
    ...
    {
        $this->invoice = Invoice::with('client', 'client.projects')->findOrFail($id)->withCustomFields();
    ...
    public function deleteAppliedCredit(Request $request, $id)
    {
        ...
        if ($this->invoice->amountPaid() == $this->invoice->due_amount) {
        ...
        $this->invoice->due_amount += $payment->amount;
    ...
    public function getClientOrCompanyName($projectID = '')
    {
        ...
        if ($projectID == '') {
            ...
            $client = Project::where('id', $projectID)->with('client')->first();
            $this->companyName = '';
            $this->clientId = '';

            if ($client) {
                $this->companyName = $client->client->name;
