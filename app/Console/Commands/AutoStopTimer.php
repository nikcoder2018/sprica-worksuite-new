    ...
    public function handle()
    {
        ...
        if ($logTimeFor->auto_timer_stop == 'yes') {
            ...
            foreach ($activeTimers as $activeTimer) {
                 $endDateTime = Carbon::createFromFormat('Y-m-d H:i:s', $activeTimer->start_time->format('Y-m-d').' '.$attendanceSetting->office_end_time)->format('H:i:s');
                 $endDateTime = Carbon::parse($endDateTime, $setting->timezone)->timestamp;
